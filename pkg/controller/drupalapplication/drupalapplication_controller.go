...
func Add(mgr manager.Manager) error {
	err := builder.
		...
		For(&fnv1alpha1.DrupalApplication{}).
		Complete()
	if err != nil {
		log.Error(err, "could not create drupalapplication webhook")
		return err
	}
...
// ^^^ Above methods are generated boilerplate and won't need to be unit tested
...
func (r *ReconcileDrupalApplication) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if rh.app.Spec.ImageRepo == "" {
		rh.app.Spec.ImageRepo = customercontainer.ECRRepoURIFromGitRepoURL(rh.app.Spec.GitRepo)

		rh.logger.Info("Setting ImageRepo field based on GitRepo", "ImageRepo", rh.app.Spec.ImageRepo)
		if err := r.client.Update(context.TODO(), rh.app); err != nil {
			return reconcile.Result{}, err
		}
		return reconcile.Result{Requeue: true}, nil
	...
	hashedGitRepo := common.HashValueForLabel(rh.app.Spec.GitRepo)
	...
	err = r.client.List(context.TODO(), drenvList, client.MatchingLabels(labels))
	if err != nil {
